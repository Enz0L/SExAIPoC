...
#Service name used to store credentials in GNOME Keyring
SERVICE_NAME = "sexai"

def get_credentials():
    key = keyring.get_password(SERVICE_NAME, "fofa_key")
    email = keyring.get_password(SERVICE_NAME, "fofa_email")
    if not key or not email:
        print("[SExAI] First run — storing FOFA credentials in GNOME Keyring.")
        email = input("FOFA email: ")
        key = getpass.getpass("FOFA API key: ")
        keyring.set_password(SERVICE_NAME, "fofa_key", key)
        keyring.set_password(SERVICE_NAME, "fofa_email", email)
    return key, email

if __name__ == "__main__":
    tab = [['IP','Port']]
    key, email = get_credentials()
    client = fofa.Client(key=key, email=email)
    query_str = input("FOFA query: ").strip()
    if not query_str:
    ...
    model_name = input("Ollama model: ").strip()
    if not model_name:
    ...
    for pair in tab[1:]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                question = HumanMessagePromptTemplate.from_template('Analyze this HTML Body please: {body}, give me all informaiton about what is contained in this one, give example and advices.')
                question
